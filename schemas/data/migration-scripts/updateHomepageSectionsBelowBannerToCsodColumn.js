...
// Run this script with: `sanity exec --with-user-token src/migrations/updateHomepageSectionsBelowBannerToCsodColumn.js`
//
...
const fetchDocuments = () => client.fetch(
  '*[_type == "homepage" && defined(sectionsBelowBanner) && sectionsBelowBanner[]._type == "twoColumnSection"][0...100] {_id, _type, _rev, sectionsBelowBanner}',
);

const buildPatches = docs => docs.reduce((acc, doc) => {
  let current = {};

  const twoColumnSectionsWithColumn = doc.sectionsBelowBanner.filter(
    (sec) => {
  ...
  if (twoColumnSectionsWithColumn.length !== 0) {
    const newSections = doc.sectionsBelowBanner.map((sec) => {
    ...
    current = {
      id: doc._id,
      patch: {
        set: { sectionsBelowBanner: newSections },
