...
const fetchDocuments = () => client.fetch(
  '*[ sections[]._type == "multiSection" && sections[].sections[]._type == "oneColumnSection" && defined(sections[].sections[].image)]',
);

const buildPatches = docs => docs.map(doc => ({
  id: doc._id,
  patch: {
    unset: ['sections[_type == "multiSection"].sections[_type == "oneColumnSection"].image'],
...
const migrateNextBatch = async () => {
  ...
  console.log('doxxxxxx:', documents.length, documents);
