...
// Run this script with: `sanity exec --with-user-token schemas/data/migrateData.js` in /backend
//
...
const fetchDocuments = () => client.fetch(
  '*[sections[]._type == "bubbleCtaSection1"][0..100] {_id, _rev, sections, bubbleCtaSection}',
);

const buildPatches = docs => docs.map((doc) => {
  const currentBubbleCta = doc.sections.filter(
    section => section._type === 'bubbleCtaSection1',
  )[0];
  return {
    id: doc._id,
    patch: {
      set: {
        bubbleCtaSection: {
          _ref: currentBubbleCta.bubbleCtaReference1._ref,
          _type: 'reference',
        },
      },
      unset: ['sections[_type == "bubbleCtaSection1"]'],
