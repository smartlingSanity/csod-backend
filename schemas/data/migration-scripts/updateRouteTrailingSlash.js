...
// as complete
const fetchDocuments = () => client.fetch('*[_type == "route"][0...100]');
...
const buildPatches = docs => docs.reduce((acc, doc) => {
  ...
  if (doc.slug.current !== 'undefined') {
    current = {
      id: doc._id,
      patch: {
        set: {
          slug: {
            current: shouldAddTrailingSlash(doc.slug.current) ? forceTrailingSlash(doc.slug.current) : doc.slug.current,
          },
        },
