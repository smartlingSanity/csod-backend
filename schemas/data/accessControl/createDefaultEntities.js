...
const defaultGroups = [
  {
    ...
    groupEntityID: 'csod.group.sanity_administrators',
  ...
  },
  {
    ...
    groupEntityID: 'csod.group.sanity_editors',
  ...
  },
  {
    ...
    groupEntityID: 'csod.group.sanity_read_only',
...
const getProject = async projectID => axios.get(
...
const getUserDetails = async (projectID, userIDs) => axios.get(
...
const updateGroups = async (runUpdateGroups) => {
  if (runUpdateGroups) {
    try {
      const transaction = client.transaction();
      transaction.reset();
      ...
      Promise.all(
        await client.fetch(`
      *[_type == 'groupEntity' && !(_id in path("drafts.**"))]{
  '_type': 'system.group',
  '_id': '_.groups.'+groupEntityID,
  'grants': groupEntityGrants[]->{
    'filter': grantEntityFilters,
    'permissions' : grantEntityPermissions
  },
  'members': groupEntityMembers[]{'id': _ref}.id,
}
`),
      )
        .then(groupEntities => groupEntities
          .map(groupEntity => transaction.createOrReplace(groupEntity)))
        .then(() => transaction.commit());
    } catch (e) {
      console.log(e);
    }
...
resetData(false).then(() => console.log('reset data done'));

initAcl(false).then(() => console.log('init acl done'));

updateGroups(false).then(() => console.log('sync groups done'));
